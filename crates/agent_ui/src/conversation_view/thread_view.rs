...
use gpui::{Bounds, List, ListOffset, Stateful, TaskExt};
...
impl ThreadView {
    ...
    fn sticky_user_message_candidate_index(
        ...
        if top_item_index >= list_item_count {
            return None;
        }
            ...
            .child(
                h_flex().w_full().justify_center().child(
                    h_flex()
                        ...
                        .when_some(max_content_width, |this, max_w| this.max_w(max_w))
                        .px_2()
                        .h(STICKY_USER_MESSAGE_HEADER_HEIGHT)
                        .child(
                            h_flex()
                                .w_full()
                                .h_full()
...
mod sticky_user_message_tests {
    ...
    #[test]
    fn sticky_user_message_stays_hidden_at_end_scroll_sentinel() {
        let trailing_item_candidate = ThreadView::sticky_user_message_candidate_index(
            4,
            5,
            |index| matches!(index, 0 | 2),
            4,
        ...
        let end_sentinel_candidate = ThreadView::sticky_user_message_candidate_index(
            4,
            5,
            |index| matches!(index, 0 | 2),
            5,
        ...
        assert_eq!(trailing_item_candidate, Some(2));
        assert_eq!(end_sentinel_candidate, None);
    }
    ...
    fn sticky_user_message_preview_uses_structured_labels_for_references() {
        ...
        let (segments, preview, has_more_message_content) =
            ThreadView::sticky_user_message_preview(&chunks, PathStyle::Unix);
